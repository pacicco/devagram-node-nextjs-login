...
import { upload, uploadImagemCosmic } from "../../middlewares/services/uploadImagemCosmic";
...
const handler = nc()
    ...
    .get (async (req: NextApiRequest, res: NextApiResponse<RespostaPadraoMsg>) => {
        ...
        return res.status(400).json({ erro: 'Nao foi possivel obter os dados do usuario' })
    });
    export const config = {
        api: {
            bodyParser : false
